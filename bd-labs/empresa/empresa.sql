CREATE TABLE funcionario (
        ...
        supervisorcpf VARCHAR(15),
        numerodept INTEGER,
        PRIMARY KEY (cpf),
        FOREIGN KEY (supervisorcpf) REFERENCES funcionario (cpf)
);

CREATE TABLE departamento (
        dnumero INTEGER NOT NULL,
        dnome VARCHAR(45),
        gerentecpf VARCHAR(15),
        dataInicio DATE,
        PRIMARY KEY (dnumero),
        FOREIGN KEY (gerentecpf) REFERENCES funcionario(cpf)
);
...
CREATE TABLE projeto (
        ...
        FOREIGN KEY (numerodepartamento)REFERENCES departamento (dnumero)
);

CREATE TABLE trabalhaem (
	funcionariocpf VARCHAR(15) NOT NULL,
        projetonumero INTEGER NOT NULL,
        horas INTEGER,
        PRIMARY KEY (projetonumero,funcionariocpf),
        FOREIGN KEY (projetonumero) REFERENCES projeto (projnumero),
...
CREATE TABLE dependente (
        ...
        PRIMARY KEY (funcpf,nomeDependente),
        FOREIGN KEY (funcpf) REFERENCES funcionario (cpf)

);


ALTER TABLE funcionario ADD CONSTRAINT empregado_depto_fk FOREIGN KEY (numerodept) REFERENCES departamento(dnumero);
...
UPDATE departamento set gerentecpf = '1230984567' where dnumero = 5;
UPDATE departamento set gerentecpf = '1029384756' where dnumero = 4;
